...
data <- read.csv('data\\Data4Shiny.csv')
EfixedeffectsTotal <- read.csv('data\\EfixedeffectsTotal.csv')
EfixedeffectsBlack <- read.csv('data\\EfixedeffectsBlack.csv')
EfixedeffectsWhite <- read.csv('data\\EfixedeffectsWhite.csv')
head(data)
...
shinyServer(function(input,output){
  ...
  output$table <- renderTable({
    if (input$adj=="Poisson Regression"&input$data=="Total Arrests"){
      mat=EfixedeffectsTotal[2:8,c(1,3,5)]
      colnames(mat)<-c("Mean","95% CI Lower Bound","95% CI Upper Bound")
      rownames(mat)<-c("% Black",
                       "% Living in Poverty",
                       "% Bachelors degree or more",
      ...
      if (input$adj=="Poisson Regression"&input$data=="White Only Arrests"){
        mat=EfixedeffectsWhite[2:8,c(1,3,5)]
        colnames(mat)<-c("Mean","95% CI Lower Bound","95% CI Upper Bound")
        rownames(mat)<-c("% Black",
                         "% Living in Poverty",
                         "% Bachelors degree or more",
        ...
        if (input$adj=="Poisson Regression"&input$data=="Black Only Arrests"){
          mat=EfixedeffectsBlack[2:8,c(1,3,5)]
          colnames(mat)<-c("Mean","95% CI Lower Bound","95% CI Upper Bound")
          rownames(mat)<-c("% Black",
                           "% Living in Poverty",
                           "% Bachelors degree or more",
